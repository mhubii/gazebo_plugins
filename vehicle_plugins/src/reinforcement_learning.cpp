...
VehiclePlugin::VehiclePlugin() :
	...
	//model_.to(device_);
	// printf("yo\n");
	// model_ = new Actor({120, 160}, DOF, 64);
	// printf("yi\n");
}
...
void VehiclePlugin::Load(physics::ModelPtr parent, sdf::ElementPtr sdf) {
	...
	sdf::ElementPtr model_sdf = this->model->GetLink("chassis")->GetSDF();
	...
	printf("hay\n");
	Actor lala({3, height, width}, DOF, 64);
	printf("hoy\n");
	// }
	...
	if (!CreateAgent()) {
		...
		printf("VehiclePlugin -- failed to create reinforcement learning agent\n");
	}
